...
bool Nano::CheckButtons(int button)
{
    for (int i = 0; i < 5; i++)
    {
        if (i == button)
            return digitalRead(buttons[i]);
    }
}
...
void Nano::LEDONandOFF (bands_t & decAVG)
{
    ...
    for (int i = 0; i < NUM_OF_COL; i++)
    {
        ...
        for (int j = 0; j < NUM_OF_ROW; j++)
        {
            ...
            if (decAVG.band[i] >= current_box_db)
            {
                int count = 0;
